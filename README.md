# Mental Health Resources for Canada

A comprehensive, free resource platform providing mental health support, crisis resources, and evidence-based recovery strategies for Canadians.
https://eltonaguiar.github.io/MENTALHEALTHRESOURCES/

## 🌟 Features

- **Emergency Resources**: Prominent crisis hotlines and immediate support options
- **Canada-Specific**: Mental health services and resources available across Canada
- **Panic Attack Guide**: Self-guided techniques and coping strategies
- **Depression Recovery**: Healthy habits, activities, and bibliotherapy recommendations
- **Accessible**: Responsive design, keyboard navigation, and inclusive content
- **Free & Open**: All resources are freely available to everyone

## 📁 Project Structure

```
MentalHealthResources/
├── index.html                              # Main landing page
├── styles.css                              # Styling for the website
├── script.js                               # Interactive features and animations
├── Canada_Mental_Health_Resources.md       # Comprehensive Canadian mental health directory
├── Panic_Attack_Self_Help_Guide.md        # Self-guided panic attack recovery techniques
├── Healthy_Habits_Depression_Recovery.md  # Depression recovery activities and wellness practices
└── README.md                               # This file
```

## 🚨 Emergency Resources

If you are in crisis, help is available immediately:

- **Canada Suicide Prevention Service**: 1-833-456-4566 (24/7)
- **Crisis Text Line**: Text HOME to 741741
- **Kids Help Phone**: 1-800-668-6868 (for youth under 20)
- **Emergency**: 911

## 📚 Resources Included

### 1. Canada Mental Health Resources
- National and provincial crisis hotlines
- Government mental health programs
- Mental health organizations and associations
- Therapy and counseling services
- Support for specific populations (LGBTQ+, Indigenous, seniors, youth)
- Substance use support resources

### 2. Panic Attack Self-Help Guide
- Immediate coping techniques (5-4-3-2-1 grounding, box breathing, etc.)
- Cognitive strategies and reality checking
- Physical interventions
- Long-term prevention strategies
- Professional treatment options

### 3. Healthy Habits & Depression Recovery
- Bibliotherapy and book recommendations
- YouTube channels for mood boost
- Physical activities (solo and group)
- Mental health practices (meditation, journaling, creative outlets)
- Nutrition and sleep optimization
- Activity planner by depression level

## 🌐 How to Use

1. **View Landing Page**: Open `index.html` in a web browser
2. **Download Guides**: Click on resource cards to download markdown files
3. **Access Emergency Resources**: Emergency hotlines are prominently displayed
4. **Share Resources**: Guides are shareable and can be distributed to anyone needing support

## 💻 Local Development

### Prerequisites
- A web browser (Chrome, Firefox, Safari, Edge, etc.)
- A text editor (optional, for modifications)

### Running Locally
1. Clone or download this repository
2. Open `index.html` in your web browser
3. No server or additional setup required

### Deployment
This static website can be deployed to:
- GitHub Pages (free)
- Netlify (free tier available)
- Vercel (free tier available)
- Any web hosting service supporting static HTML

## 📝 Content Guidelines

All content is:
- ✓ Evidence-based and research-backed
- ✓ Canada-focused where applicable
- ✓ Inclusive and accessible
- ✓ Regularly updated
- ✓ Peer-reviewed recommendations

## 🤝 Contributing

We welcome contributions! Areas for improvement:
- Additional provincial resources
- More book and video recommendations
- Translation support
- Accessibility enhancements
- Bug reports and suggestions

To contribute:
1. Fork the repository
2. Create a feature branch
3. Make your changes
4. Submit a pull request

## 📄 License

This project is open source and available for anyone to use, modify, and distribute for mental health support purposes.

## ⚠️ Important Disclaimer

These resources are provided for educational and support purposes. They are not a substitute for professional medical advice, diagnosis, or treatment. Always consult with a qualified mental health professional for personalized advice.

**In Crisis?** Contact emergency services or a crisis helpline immediately:
- **Call**: 1-833-456-4566 or 911
- **Text**: HOME to 741741

## 🙏 Acknowledgments

Resources compiled from:
- Anxiety Canada
- Canadian Mental Health Association
- Government of Canada Health Services
- Psychology Today
- Mental Health America
- Evidence-based mental health research

## 📧 Support

For questions or suggestions about these resources, please reach out or submit an issue through GitHub.

---

**Last Updated**: January 18, 2026

**Remember**: You are not alone. Help is available, and recovery is possible.
